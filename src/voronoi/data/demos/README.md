# Demos

Example scenarios to test the agent swarm orchestrator. Each demo includes a ready-to-use prompt and step-by-step instructions.

| Demo | Description | Agents | Waves |
|------|-------------|--------|-------|
| [Computational Triage](computational-triage/) | Context engineering scaling axis — single-agent replication + multi-agent amplification | 8+ | 4 |
| [Compilation-Threshold Hunt](compilation-threshold-hunt/) | Same hypothesis as Epistemic Trajectories — swarm designs the experiment, surprise-budget protocol | 8+ | adaptive |
| [Coupled Decisions](coupled-decisions/) | Multi-agent reasoning over coupled commercial levers | 8+ | 4 |
| [Epistemic Trajectories](epistemic-trajectories/) | Knowledge compilation + cross-source constraint detection + feasible trajectory pruning | 8+ | 4 |
| [Forgetting Cure](forgetting-cure/) | Brain-inspired anti-forgetting strategies — computational neuroscience | 9 | 4 |
| [Emergent Ecosystem](emergent-ecosystem/) | Multi-species simulation with emergent communication | 6 | 3 |
| [Attractor Regimes](ei-agents/) | Metastable band between consensus and independence in a clustered society of frozen SLMs — needs an OpenAI-compatible endpoint with logprobs | 8–16 | adaptive |

## Running a Demo

### CLI (recommended)

```bash
voronoi demo run emergent-ecosystem
```

### Interactive (human-in-the-loop)

```bash
voronoi init
copilot
> /swarm Build from demos/emergent-ecosystem/PROMPT.md
```
