...
Question Framer and Assumption Auditor are auxiliary gate/audit roles, not substitutes for the core science roles. Question Framer is never invoked in PROVE because PROVE question immutability is part of the pre-registration contract (INV-54) and is structurally enforced by the locked-claim fidelity gate (INV-59, SCIENCE.md §24). Assumption Auditor is never invoked in DISCOVER because DISCOVER can still reframe before investigation; in PROVE it records hidden assumptions for later review without blocking or revising the question. If a `preconditions` slot of the locked claim cannot be satisfied by the data, agents MUST follow the STOP-on-precondition path (escalate to the user, do not silently rescope) rather than treat it as an assumption to audit.
