...
### INV-60: Claims Are Propositions, Not Tasks
...
This prose-level invariant is structurally enforced by the locked-claim schema (`voronoi.science.locked_claim`, SCIENCE.md §24) and by INV-59. The locked claim is the machine-checkable contract; deviations require either a STOP-on-precondition escalation to the user, or an explicit re-lock that auto-demotes the investigation to DISCOVER. Agents MUST NOT silently re-derive the question to fit available data.

### INV-55: Title Laundering Rejected at Create-Time
`voronoi.mcp.tools_beads.create_task` MUST reject any title whose first non-whitespace token matches the bare-imperative set (`Analyze`, `Analyse`, `Investigate`, `Run`, `Check`, `Explore`, `Examine`, `Study`, `Review`, `Assess`, `Evaluate`, `Test`, `Verify`, `Look`, `Find`, `Identify`, `Determine`, `Survey`) AND that contains no relational marker (`>`, `<`, `=`, `vs`, `causes`, `predicts`, `increases`, `decreases`, `is`, `are`, …). This mirrors the Claim Ledger's `validate_claim_statement` (INV-60, claim shape) but enforces it at the *task creation* boundary instead of at the post-hoc claim sync. Without this gate, ghost tasks like `"Analyze business prompt findings"` are dispatchable, burn worker time, and silently disappear at the ledger layer. The dispatcher's substring guard remains as a defence-in-depth layer.
...
### INV-59: Locked-Claim Fidelity at Merge (PROVE mode)
PROVE-mode investigations MUST persist a five-slot `LockedClaim` (`claim`, `scope`, `decision_rule`, `falsifier`, `preconditions`) to `.swarm/locked-claim.json` at intake. Agents MUST emit `.swarm/executed-claim.json` from the claim ledger at the same point they emit the run manifest. Before merge, the dispatcher MUST call `voronoi.science.locked_claim.compare_claims(locked, executed)` and on `passed=False` MUST emit a `locked_claim_divergence` directive and halt merge. The only override path is explicit user approval that auto-demotes the investigation to DISCOVER (per INV-54). When agents discover a `preconditions` slot cannot be satisfied by the available data, they MUST follow the STOP-on-precondition-failure path (SCIENCE.md §24.4) — write a `precondition_violation` event into the claim ledger and escalate to the user with structured options. Silent rescoping of the claim to fit data availability is forbidden. This is the structural counterpart to INV-54 (prose-level question immutability) and is complementary to EviBound's artifact-level evidence binding (Chen 2025, arXiv:2511.05524).
