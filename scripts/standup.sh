...
echo ""
echo "Open tasks:"
bd list --status open --json 2>/dev/null | jq -r '.[] | "  [\(.id)] P\(.priority) \(.title)"' 2>/dev/null || bd list --status open
echo ""
echo "In progress:"
bd list --status in_progress --json 2>/dev/null | jq -r '.[] | "  [\(.id)] \(.assignee // "unassigned") — \(.title)"' 2>/dev/null || bd list --status in_progress
echo ""
echo "Ready (unblocked):"
bd ready 2>/dev/null || echo "  (none or bd ready not available)"
echo ""
echo "Recently closed (24h):"
bd list --status closed --json 2>/dev/null | jq -r '.[] | "  [\(.id)] \(.title)"' 2>/dev/null || bd list --status closed
echo ""
